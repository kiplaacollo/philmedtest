table 454 "Approval Entry"
{
    ...
    fields
    {
        ...
        field(2; "Document Type"; Option)
        {
            ...
            OptionCaption = 'Quote,Order,Invoice,Credit Memo,Blanket Order,Return Order,Leave,AssetTransfer,AssetUploading,Trans';
            OptionMembers = Quote,"Order",Invoice,"Credit Memo","Blanket Order","Return Order",Leave,AssetTransfer,AssetUploading,Trans;
        }
        ...
        {
            Caption = 'Status';
            OptionCaption = 'Created,Open,Canceled,Rejected,Approved,Posted';
            OptionMembers = Created,Open,Canceled,Rejected,Approved,Posted;
            ...
            begin
                ...
                if "Document Type" = "Document Type"::Order then begin
                    if usersetup.Get("Approver ID") then begin
                        if Status = Status::Open then begin
                            Emailmessage := 'You have a Purchase Order approval request ' +
                            '<a class="btn btn-info" role="button" href=''https://erp.mawingu.co/BC140/?page=654&filter=%27Approval%20Entry%27.Status%20IS%20%271%27&company=MAWINGU%20NETWORKS%20LTD&dc=0''>Click here to Approve</a>';
                            SMTPMail.CreateMessage('requisitions@mawingu.co', 'requisitions@mawingu.co', usersetup."E-Mail", "Document No.", Emailmessage, true);
                ...
                if "Document Type" = "Document Type"::AssetTransfer then begin
                    if usersetup.Get("Approver ID") then begin
                        if Status = Status::Open then begin
                            Emailmessage := 'You have an Asset Transfer approval request ' +
                            '<a class="btn btn-info" role="button" href=''https://erp.mawingu.co/BC140/?page=654&filter=%27Approval%20Entry%27.Status%20IS%20%271%27&company=MAWINGU%20NETWORKS%20LTD&dc=0''>Click here to Approve</a>';
                            SMTPMail.CreateMessage('requisitions@mawingu.co', 'requisitions@mawingu.co', usersetup."E-Mail", "Document No.", Emailmessage, true);
                            SMTPMail.AddCC('ops@mawingu.co');
                            SMTPMail.SendCustom('requisitions@mawingu.co', 'Tat44669');
                ...
                if "Document Type" = "Document Type"::AssetUploading then begin
                    if usersetup.Get("Approver ID") then begin
                        if Status = Status::Open then begin
                            Emailmessage := 'You have an Asset Upload request to approve ' +
                            '<a class="btn btn-info" role="button" href=''https://erp.mawingu.co/BC140/?page=654&filter=%27Approval%20Entry%27.Status%20IS%20%271%27&company=MAWINGU%20NETWORKS%20LTD&dc=0''>Click here to Approve</a>';
                            SMTPMail.CreateMessage('requisitions@mawingu.co', 'requisitions@mawingu.co', usersetup."E-Mail", "Document No.", Emailmessage, true);
                            SMTPMail.AddCC('ops@mawingu.co');
                            SMTPMail.SendCustom('requisitions@mawingu.co', 'Tat44669');
                ...
                if Status = Status::Rejected then begin
                    if "Document Type" = "Document Type"::AssetTransfer then begin
                        AssetTransfer.Reset;
                        AssetTransfer.SetRange(AssetTransfer."Document No", "Document No.");
                        if AssetTransfer.Find('-') then begin
                            AssetTransfer.Status := AssetTransfer.Status::Rejected;
                            AssetTransfer.Modify;
                            Message('Asset Transfer %1  has been Rejected successfully', "Document No.");
                        end;
                    end;

                    //Asset Uploading
                    if "Document Type" = "Document Type"::AssetUploading then begin
                        AssetUploading.Reset;
                        AssetUploading.SetRange(AssetUploading."Document No", "Document No.");
                        if AssetUploading.Find('-') then begin
                            AssetUploading.Status := AssetUploading.Status::Rejected;
                            AssetUploading.Modify;
                            Message('Asset Uploading %1  has been Rejected successfully', "Document No.");
                        end;
                    end;
                end;
            end;
        ...
        field(32; "Leave Staff  No"; Code[26])
        {
    ...
    var
        ...
        employeee: Record pr_employees;
        usersetup: Record "User Setup";
        Emailmessage: Text;
        AssetTransfer: Record "Asset Transfer Table";
        AssetUploading: Record "Stock Submodule";
        ...
        case "Table ID" of
            ...
            DATABASE::"HR Employee Requisitions":
                begin
                    ...
                    CustVendorNo := requisition."Job ID";
                    CustVendorName := requisition."Job Description";
                end;
            ...
            DATABASE::"Asset Transfer Table":
                begin
                    RecRef.SetTable(AssetTransfer);
                    CustVendorNo := AssetTransfer."Document No";
                    CustVendorName := AssetTransfer."Item Name";
                end;

            DATABASE::"Stock Submodule":
                begin
                    RecRef.SetTable(AssetUploading);
                    CustVendorNo := AssetUploading."Document No";
                    CustVendorName := AssetUploading."Item Name";
                end;
