table 9054 "Finance Cue"
{
    ...
    fields
    {
        ...
        field(34; Items; Decimal)
        {
            CalcFormula = Sum ("Item Ledger Entry".Quantity WHERE (Quantity = FILTER (<> 0)));
            FieldClass = FlowField;
        }
        field(35; "New Items"; Decimal)
        {
            CalcFormula = Sum ("Item Ledger Entry".Quantity WHERE ("Variant Code" = CONST ('NEW'),
                                                                  Quantity = FILTER (<> 0)));
            FieldClass = FlowField;
        }
        field(36; "Used Items"; Decimal)
        {
            CalcFormula = Sum ("Item Ledger Entry".Quantity WHERE ("Variant Code" = CONST ('USED'),
                                                                  Quantity = FILTER (<> 0)));
            FieldClass = FlowField;
        }
        field(37; "Approved Transfer orders"; Integer)
        {
            CalcFormula = Count ("Transfer Header" WHERE (Status = CONST (Approved)));
            FieldClass = FlowField;
        }
        field(38; "Pending approval TOs"; Integer)
        {
            CalcFormula = Count ("Transfer Header" WHERE (Status = CONST ("Pending Approval")));
            FieldClass = FlowField;
        }
        field(39; "Shipment TOs"; Integer)
        {
            CalcFormula = Count ("Transfer Shipment Header");
            FieldClass = FlowField;
        }
        field(40; "Receipt TOs"; Integer)
        {
            CalcFormula = Count ("Transfer Receipt Header");
            FieldClass = FlowField;
        }
        field(41; WareHouse; Decimal)
        {
            CalcFormula = Sum ("Item Ledger Entry".Quantity WHERE ("Location Code" = CONST ('NANYUKI'),
                                                                  Quantity = FILTER (<> 0)));
            FieldClass = FlowField;
        }
        field(42; "items to install"; Integer)
        {
            CalcFormula = Count ("Items to Install" WHERE ("Date of Installation" = FIELD ("Due Date Filter")));
            FieldClass = FlowField;
        }
    }
