table 14 Location
{
    ...
    fields
    {
        ...
        field(5720; "Country/Region Code"; Code[30])
        {
        ...
        field(7305; "Put-away Template Code"; Code[30])
        {
        ...
        field(7701; "Is Store"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(7702; IsBaseStation; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(7703; "SuperVisor Staff No"; Code[30])
        {
            DataClassification = ToBeClassified;
            ...
            begin
                ...
                Payroll.Reset;
                Payroll.SetRange(Payroll.st_no, "SuperVisor Staff No");
                if Payroll.Find('-') then
                    "Supervisor Name" := Payroll.Name;
                Modify;
            end;
        }
        field(7704; "Supervisor Name"; Code[60])
        {
            DataClassification = ToBeClassified;
        }
    }
    ...
    var
        ...
        UnspecifiedLocationLbl: Label 'Base Stations';
        Payroll: Record pr_employees;

    procedure RequireShipment(LocationCode: Code[50]): Boolean
    begin
    ...
    procedure RequirePicking(LocationCode: Code[50]): Boolean
    begin
        if Location.Get(LocationCode) then
    ...
    procedure RequireReceive(LocationCode: Code[50]): Boolean
    begin
        if Location.Get(LocationCode) then
    ...
    procedure RequirePutaway(LocationCode: Code[50]): Boolean
    begin
        if Location.Get(LocationCode) then
    ...
    procedure GetLocationSetup(LocationCode: Code[50]; var Location2: Record Location): Boolean
    begin
    ...
    procedure IsInTransit(LocationCode: Code[50]): Boolean
    begin
        if Location.Get(LocationCode) then
    ...
    begin
        ...
        if not IncludeOnlyUnspecifiedLocation then begin
            if ExcludeInTransitLocations then
                ...
                Location.SetRange(Location."Is Store", true);
