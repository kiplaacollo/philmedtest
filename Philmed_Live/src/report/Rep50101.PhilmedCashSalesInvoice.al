report 50101 "Philmed Cash Sales - Invoice"
{
    RDLCLayout = './PhilmedCashSalesInvoice.rdl';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(CustomerCurrentBalance; CustomerCurrentBalance)
            {
            }
            ...
            column(Posting_Description; "Posting Description")//AMI
            {
            }
            column(Cust_Phone_No; Cust."Phone No.")//AMI
            {
            }
            column(Urgent_Comments; "Urgent Comments")//AMI
            {
            }
            column(NoOfCopyLable; NoOfCopyLable)//AMI
            {
            }
            column(MPESATillNo; MPESATillNo)//AMI
            {
            }
            column(BranchCode; "Shortcut Dimension 1 Code")//AMI
            {
            }
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(Batch_No_; "Batch No.")//AMI
                {
                }
                column(Expiry_Date; "Expiry Date")//AMI
                {
                }
            ...
            var
                ...
                lvCustomer: Record Customer;//AMI
                lvDimensionValue: Record "Dimension Value";//AMI
            ...
            begin
                ...
                if NOT lvUserSetup."Allow Invoice Re-Print" then if "No. Printed" > 0 then error('You do not have permission to reprint');
                ...
                //AMI Philmed Customer Balance
                lvCustomer.get("Bill-to Customer No.");
                lvCustomer.CalcFields("Balance (LCY)", "Current Balance");
                //CustomerCurrentBalance := lvCustomer."Balance (LCY)";//To be activated after Balance Corrections
                CustomerCurrentBalance := lvCustomer."Current Balance";
                If CustomerCurrentBalance < 0 then//Hold showing Negative Balances
                    CustomerCurrentBalance := 0;

                If "No. Printed" <= 1 then
                    NoOfCopyLable := 'ORIGINAL'
                else
                    NoOfCopyLable := 'PRINT COPY - ' + Format("No. Printed" - 1);

                //Till Number
                lvDimensionValue.SetRange("Global Dimension No.", 1);
                lvDimensionValue.SetRange(Code, "Shortcut Dimension 1 Code");
                if lvDimensionValue.FindFirst() then begin
                    If lvDimensionValue."MPESA Till No." <> '' Then
                        MPESATillNo := 'TILL NO.: ' + lvDimensionValue."MPESA Till No."
                    else
                        MPESATillNo := '';
                end else begin
                    MPESATillNo := '';
                ...
                // "QR Code" := BarcodeProvider.EncodeFont(Barcode, BarcodeSym);
    ...
    var
        ...
        CustomerCurrentBalance: Decimal;  //AMI
        NoOfCopyLable: Text[20];
        MPESATillNo: Code[20];
    ...
    local procedure GenerateQRCode(No: Code[250])
    var
         Url: Text;
        Client: HttpClient;
    ...
    begin
        ...
        if SalesHeader.Find('-') then begin
            url := 'https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=' + SalesHeader."QR Code";
            if Client.Get(url, Response) then begin
                if Response.IsSuccessStatusCode() then begin
                    Response.Content().ReadAs(Ins);
                    SalesHeader."Kra QR Code".CreateOutStream(OutS);
                    CopyStream(OutS, Ins);
                    SalesHeader.Modify();
